	...
	<div>
		...
		<div class="bd bd-top">
			<div class="singer-info-hd">
				<img v-bind:src="replaceUrl(info.imgurl)"/>
			</div>
			<ul class="panel-songslist">
				<li class="panel-songslist-item" v-for="item in list">
...
<script>
	import {getLastSingerInfo} from '@/server'
	...
	export default {
		...
		computed: {
			getDetail() {
				getLastSingerInfo(this.$route.params.infoId)
					.then((res) => {
						this.info = res.info
						this.list = res.songs.list
						console.log(res)
					})
					.catch((error) => {
						console.log(error)
